...
（列出需要关注的风险点；最后补充“建议仅供参考，不构成投资建议”。）"""
        ...
        add_section(
            "消息催化",
            "（标题固定，不得改名。结合近三日新闻和指数表现，提炼真正影响明日交易的催化或扰动；不要推断未提供的资金流、市场宽度或板块榜）",
        )
        add_section(
            "明日交易计划",
            (
                "（标题固定，不得改名。必须输出字段：明日策略；仓位建议区间；重点观察方向；"
                "回避方向；触发失效条件；对持仓的动作建议（只写观察/控制仓位/等待确认，不直接喊买卖））"
            ),
        )
        add_section("风险提示", "（标题固定，不得改名。列出风险点；最后补充“建议仅供参考，不构成投资建议”。）")
        joined = "\n\n".join(sections)
        if self.profile.has_sector_rankings or self.profile.has_market_stats:
            return "【强制结构】必须严格使用以下标题，不得省略，不得改名。\n\n" + joined
        return joined
